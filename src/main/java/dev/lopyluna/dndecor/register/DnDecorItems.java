...
public class DnDecorItems {
    ...
    public static final ItemEntry<FullBeltConnectorItem> BELT_CONNECTOR = REGISTRATE.item("belt_connector", FullBeltConnectorItem::new)
   ...
   // public static final ItemEntry<ColoredStorageContainerItem> STORAGE_CONTAINERS = REGISTRATE.item("storage_container", p -> new ColoredStorageContainerItem(p, BLUE))
   //      //   .lang("Storage Container")
   //         .recipe((c, p) -> ShapedRecipeBuilder.shaped(RecipeCategory.BUILDING_BLOCKS, c.get(), 4)
   //                 .pattern("CC").pattern("CC")
   //                 .define('C', AllBlocks.ITEM_VAULT)
   //                 .unlockedBy("has_" + c.getName(), has(c.get())).save(p, DnDecor.asResource("crafting/" + c.getName()))
   //         ).model((c, p) -> p.withExistingParent("item/" + c.getName(), DnDecor.asResource("item/blue_storage_container")))
   //         .register();
