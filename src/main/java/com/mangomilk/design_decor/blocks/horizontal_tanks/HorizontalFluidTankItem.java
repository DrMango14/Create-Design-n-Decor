...
public class HorizontalFluidTankItem extends BlockItem {

	public HorizontalFluidTankItem(Block p_i48527_1_, Properties p_i48527_2_) {
	...
	private void tryMultiPlace(BlockPlaceContext ctx) {
		...
		if (!HorizontalFluidTankBlock.isBarrel(placedOnState))
			return;
		HorizontalFluidTankBlockEntity tankAt = ConnectivityHandler.partAt(CDDBlockEntities.HORIZONTAL_FLUID_TANK.get(), world, placedOnPos);
		if (tankAt == null)
			return;
		HorizontalFluidTankBlockEntity controllerBE = tankAt.getControllerBE();
		...
		int tanksToPlace = 0;
		Axis vaultBlockAxis = HorizontalFluidTankBlock.getFermentationBarrelAxis(placedOnState);
